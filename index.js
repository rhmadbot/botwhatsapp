...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285251159773@s.whatsapp.net"] 
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'bug':
                    ...
                    client.sendMessage('6288245621406@s.whatsapp.net', options, text, {quoted: mek})
				...
				case 'ytmp4':
					...
					break
                case 'text3d':
              	    if (args.length < 1) return reply('teksnya mana oyy?')
                    teks = `${body.slice(8)}`
       ...
       client.sendMessage(from, 'Tuh nomer owner ku >_<, Kalo mau masukin bot ke grub atau mau donasi Chat aja',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
